  ...
  command error_t StdControl.stop() {
   ...
   /* The routing table keeps info about neighbor's backpressure,
    ...
    uint8_t routingTableFind(am_addr_t neighbor_p) {
      uint8_t i;
      if (neighbor_p == TOS_BCAST_ADDR)					//TOS_BCAST_ADDR is a special address signifying that any node that hears the message will handle it.
    ...
    error_t routingTableUpdateEntry(am_addr_t from_p, uint16_t backpressure_p)    {
        ...
        if (idx == ROUTING_TABLE_SIZE) {
            ...
            atomic {
              ...
              routingTable[idx].linkETX = 1*100;         // Initialize to losless link
    ...
    atomic{
      ...
      for (compareIdx = 0; compareIdx < routingTableActive; compareIdx++) {
        ...
        if( routingTable[compareIdx].isBurstyNow == 1 )
        {
          ...
          if(  !burstNeighborFound || bestBurstNeighborBackpressure > routingTable[compareIdx].backpressure )				//NeighborBackpressure 要尽可能小～～
          { 
          ...
          longLinkETX = LINK_LOSS_V * routingTable[compareIdx].linkETX / 100;						//原linkETX已经乘了100
          longLinkRate = 10000 / routingTable[compareIdx].linkPacketTxTime; // Packets per minute	－－－－－－－－－ Packets per 10s --doesn't matter
      ...
      if( burstNeighborFound && routingTable[maxWeightIdx].backpressure >= bestBurstNeighborBackpressure )				//同理如上,NeighborBackpressure 要尽可能小～～
      {
        // We'll use this burst neighbor, reset the isBurstyNow which we cleared for all bursty links
        maxWeight = (int32_t)localBackpressure_p - (int32_t)bestBurstNeighborBackpressure - LINK_LOSS_V;					//假设突发neighbor的linkETX＝1；（因为无LinkETX和LinkRate）
        if( maxWeight > 0 )
        {
